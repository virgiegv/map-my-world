...
def test_get_category_by_name_two_result_two_member_list(db_session) -> None:
    ...
    category = crud.create_category(db_session, schemas.CategoryCreate(name="testCategory"))
    second_category = crud.create_category(db_session, schemas.CategoryCreate(name="testCategory2"))
    result = crud.get_category_by_name(db_session, "testCategory")
    assert ((category["detail"].name == result[0].name) and
            (category["detail"].id == result[0].id) and
            (second_category["detail"].name == result[1].name) and
            (second_category["detail"].id == result[1].id))
...
def test_get_all_categories(db_session) -> None:
    ...
    category = crud.create_category(db_session, schemas.CategoryCreate(name="testCategory"))
    second_category = crud.create_category(db_session, schemas.CategoryCreate(name="testCategory2"))
    third_category = crud.create_category(db_session, schemas.CategoryCreate(name="testCategory3"))
    result = crud.get_all_categories(db_session)
    assert ((category["detail"].name == result[0].name) and
            (category["detail"].id == result[0].id) and
            (second_category["detail"].name == result[1].name) and
            (second_category["detail"].id == result[1].id) and
            (third_category["detail"].name == result[2].name) and
            (third_category["detail"].id == result[2].id))
...
def test_create_simple_location(db_session) -> None:
    ...
    category = crud.create_category(db_session, schemas.CategoryCreate(name="testCategory"))
    result = crud.create_location(db_session,
                                  schemas.LocationCreate(name="testLocation", latitude=246, longitude=93456),
                                  category["detail"].id)
    assert ((result["detail"].name == "testLocation") and (result["detail"].latitude == 246) and (
            result["detail"].longitude == 93456))


def test_get_simple_location_by_id(db_session) -> None:
    """Test that checks if a single location can be obtained through the api using its id"""
    category = crud.create_category(db_session, schemas.CategoryCreate(name="testCategory"))
    location = crud.create_location(db_session,
                                    schemas.LocationCreate(name="testLocation", latitude=2257, longitude=43216),
                                    category["detail"].id)
...
def test_get_simple_location_by_incorrect_id(db_session) -> None:
    """Test that checks that when the id that was asked for doesnt exist, the result is none"""
    category = crud.create_category(db_session, schemas.CategoryCreate(name="testCategory"))
    crud.create_location(db_session, schemas.LocationCreate(name="testLocation", latitude=2257, longitude=43216),
                         category["detail"].id)
...
def test_get_location_by_name_one_result_one_member_list(db_session) -> None:
    ...
    category = crud.create_category(db_session, schemas.CategoryCreate(name="testCategory"))
    crud.create_location(db_session, schemas.LocationCreate(name="testLocation", latitude=27, longitude=426),
                                    category["detail"].id)
...
def test_get_location_by_name_one_result_two_member_list(db_session) -> None:
    ...
    category = crud.create_category(db_session, schemas.CategoryCreate(name="testCategory"))
    location = crud.create_location(db_session,
                                    schemas.LocationCreate(name="testLocation", latitude=27, longitude=426, ),
                                    category["detail"].id)
    crud.create_location(db_session,
                         schemas.LocationCreate(name="wrongEntry", latitude=5427, longitude=42875686),
                         category["detail"].id)
    result = crud.get_location_by_name(db_session, "testLocation")
...
def test_get_location_by_name_two_result_two_member_list(db_session) -> None:
    ...
    location = crud.create_location(db_session,
                                    schemas.LocationCreate(name="testLocation", latitude=27, longitude=426, ),
                                    category["detail"].id)
    second_location = crud.create_location(db_session, schemas.LocationCreate(name="testLocation2", latitude=5427,
                                                                              longitude=42875686),
                                           category["detail"].id)
    result = crud.get_location_by_name(db_session, "testLocation")
    assert ((location["detail"].name == result[0].name) and
            (location["detail"].id == result[0].id) and
            (second_location["detail"].name == result[1].name) and
            (second_location["detail"].id == result[1].id))


def test_get_location_by_name_zero_result_two_member_list(db_session) -> None:
    """Test that checks if a obtaining no results from looking in the category table responds appropriately"""
    category = crud.create_category(db_session, schemas.CategoryCreate(name="testCategory"))
    crud.create_location(db_session, schemas.LocationCreate(name="testLocation", latitude=5427, longitude=42875686),
                         category["detail"].id)
    crud.create_location(db_session, schemas.LocationCreate(name="testLocation2", latitude=5427, longitude=42875686),
                         category["detail"].id)
    result = crud.get_category_by_name(db_session, "badEntry")
    assert (len(result) == 0)
...
def test_get_all_locations(db_session) -> None:
    """Test that checks if one can get all locations from a table with 3 locations"""
    category = crud.create_category(db_session, schemas.CategoryCreate(name="testCategory"))
    location = crud.create_location(db_session,
                                    schemas.LocationCreate(name="testLocation", latitude=27, longitude=426, ),
                                    category["detail"].id)
    second_location = crud.create_location(db_session, schemas.LocationCreate(name="testLocation2", latitude=5427,
                                                                              longitude=42875686),
                                           category["detail"].id)
    third_location = crud.create_location(db_session, schemas.LocationCreate(name="testLocation3", latitude=5432427,
                                                                             longitude=428771), category["detail"].id)
    result = crud.get_all_locations(db_session)
    assert ((location["detail"].name == result[0].name) and
            (location["detail"].id == result[0].id) and
            (second_location["detail"].name == result[1].name) and
            (second_location["detail"].id == result[1].id) and
            (third_location["detail"].name == result[2].name) and
            (third_location["detail"].id == result[2].id))


def test_update_location(db_session) -> None:
    """Test that checks if one can update a single location"""
    category = crud.create_category(db_session, schemas.CategoryCreate(name="testCategory"))
    category2 = crud.create_category(db_session, schemas.CategoryCreate(name="testCategory2"))
    location = crud.create_location(db_session, schemas.LocationCreate(name="testLocation", latitude=27, longitude=426),
                                    category["detail"].id)
    crud.update_location(db_session, schemas.LocationUpdate(name="UpdatedTestLocation", latitude=1228, longitude=6222,
                                                            category_id=category2["detail"].id), location["detail"].id)
    result = crud.get_location(db_session, location["detail"].id)
...
def test_delete_location(db_session) -> None:
    """Test that checks if one can delete a single location"""
    category = crud.create_category(db_session, schemas.CategoryCreate(name="testCategory"))
    location = crud.create_location(db_session, schemas.LocationCreate(name="testLocation", latitude=27, longitude=426),
                                    category["detail"].id)
